...
namespace SigPlus_Modified
{
   public partial class Form1 : Form
   {
      ...
      private void LoadDatabase()
      {
         ...
         using (OleDbConnection connection = new OleDbConnection(@"Provider=Microsoft.ACE.OLEDB.12.0; Data Source=" + fileChosen + "; Jet OLEDB:Database Password= " + password))
         using (OleDbCommand command = new OleDbCommand("SELECT * FROM VisitorsTable ORDER BY TimeStamp;", connection))
         {
            connection.Open();
            try
            {
      ...
      private void sigPlusNET1_PenUp(object sender, EventArgs e)
      {
         ...
         else if (sigPlusNET1.KeyPadQueryHotSpot(20) > 0) // ok button for other
         {
            ...
            if (sigPlusNET1.NumberOfTabletPoints() > 0)
            {
               ...
               // CUT FROM HERE
               using (OleDbConnection connection = new OleDbConnection(@"Provider=Microsoft.ACE.OLEDB.12.0; Data Source=" + fileChosen + "; Jet OLEDB:Database Password= " + password))
               using (OleDbCommand command = new OleDbCommand("INSERT INTO VisitorsTable ([TimeStamp], [NameByteString], [CompanyRepresentedByteStream], [CitizenOrResident], [Visitee], [OtherPerson], [Chaperone]) VALUES (@timeStamp, @nameString, @companyString, @citizen, @visitee, @otherPerson, @chaperone);", connection))
               {
                  connection.Open();
                  try
                  {
      ...
      private void repetitiveMethod()
      {
         ...
         // CUT FROM HERE
         using (OleDbConnection connection = new OleDbConnection(@"Provider=Microsoft.ACE.OLEDB.12.0; Data Source=" + fileChosen + "; Jet OLEDB:Database Password= " + password))
         using (OleDbCommand command = new OleDbCommand("INSERT INTO VisitorsTable ([TimeStamp], [NameByteString], [CompanyRepresentedByteStream], [CitizenOrResident], [Visitee], [Chaperone]) VALUES (@timeStamp, @nameString, @companyString, @citizen, @visitee, @chaperone);", connection))
         {
            connection.Open();
            try
            {
